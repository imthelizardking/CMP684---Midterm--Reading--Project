...
temp_tm_init = calcTransform(q(1,:,1)); temp_tm_final = calcTransform(q_final);
pose_init = calcPose(temp_tm_init); pose_final = calcPose(temp_tm_final);
for j=1:10
    q(j,:,1) = q(1,:,1);
end
%% Loops:
for k=1:nSimDuration-1
    ...
    while bExit==false
        ...
        Phi(:,:,k,i) = [ pinv(J(:,:,k,i)),   zeros(size(inv(J(:,:,k,i))));...
                        -pinv(J(:,:,k+1,i)), pinv(J(:,:,k+1,i))];
        temp_Y = calcTransform(q(k,:,i-1));      
        Y(k,:,i) = calcPose(temp_Y);
        delta_Q(:,:,k,i) = inv(Phi(:,:,k,i)) * ([Y_star(k,:)';Y_star(k,:)']-E*Y(k,:,i)');
